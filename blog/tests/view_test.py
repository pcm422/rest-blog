...
class BlogViewTestCase(TestCase):
    def setUp(self):
        ...
        Blog.objects.create(title='배포', content='본문', author=user,)
        future_published_at = timezone.now() + timedelta(days=31)
        Blog.objects.create(title='아직 배포 안됨', content='본문', author=user, published_at=future_published_at)
